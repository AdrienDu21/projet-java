...
record Produit(String id, String nom, double prix, String categorie, int stock) {}
...
public class BoutiqueService {
    ...
    public Map<String, List<String>> getNomsParCategorie() {
        // j'ai un peu galéré sur celle là avec le groupingBy
        return produits.stream()
    ...
    public Optional<Produit> getProduitLePlusCher() {
        return produits.stream().max(Comparator.comparingDouble(Produit::prix));
    }
    public double getValeurTotaleStock() {
        return produits.stream().mapToDouble(p -> p.prix() * p.stock()).sum();
    }
}